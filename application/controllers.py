    ...
    if tickets_booked_by_user == []:
        ...
        for ticket_booked_by_user in tickets_booked_by_user:
            ...
            if (show_start_time >= datetime.now()):
                ...
                dict["booking_id"] = ticket_booked_by_user.booking_id
                dict["show"]  = str(show_venu[1]) + "(" + str(show_venu[2]) + ")" + " (past)" #show name with show language
                dict["venu"] = str(show_venu[3]) + " " + str(show_venu[4])  #venu name with venu place
                dict["venu_location"] = show_venu[5]   #venu location
                dict["show_timming"] = str(show_venu[6])[11:16] + "pm |" + str(show_venu[6])[:10]
                dict["image_path"] = show_venu[7]
...
def ticket_booking(id):
    ...
    for show_venu in show_venus:
        dynamic = Dynamic.query.filter_by(update_id = show_venu[3]).first()
        if dynamic == None:
            update_id = show_venu[3]
            seat_left = show_venu[4]
            current_price = show_venu[5]
            record = Dynamic(update_id,seat_left,current_price)
            db.session.add(record)
            db.session.commit()
            show_venu_id_with_current_price[show_venu[3]] = current_price
    ...
    for show_venu in show_venus:
        ...
        if (show_start_time >= datetime.now()):
            dynamic = Dynamic.query.filter_by(update_id = show_venu[3]).first()
            starting_price_of_ticket =show_venu[7]
            show_start_time = show_venu[6]
            total_seats= show_venu[4]
            ...
            db.session.commit()
            show_venu_id_with_current_price[show_venu[3]] = update_price
    ...
    for show_venu in show_venus:
        ...
        if (show_start_time >= datetime.now()):
            ...
            seat_restriction[show_venu[3]] = dict
            

    return render_template("show_page.html",show_id = id,show = show,user=user,check_availablity=check_availablity,venu_with_places=venu_with_places,no_show=no_show,rate_template=rate_template,show_venu_id_with_current_price=show_venu_id_with_current_price,seat_restriction=seat_restriction)
